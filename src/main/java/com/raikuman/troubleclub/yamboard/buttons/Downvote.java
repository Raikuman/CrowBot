...
public class Downvote implements ButtonComponent {

    private final YamboardManager yamboardManager;
    private final Emoji emoji;

    public Downvote(YamboardManager yamboardManager, Emoji emoji) {
    ...
    public void handle(ButtonInteractionEvent ctx) {
        yamboardManager.handleKarma(ctx.getUser(), ctx.getMessage(), false, false);
        ctx.deferEdit().queue();
    }
